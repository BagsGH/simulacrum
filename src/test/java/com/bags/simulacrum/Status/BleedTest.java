...
public class BleedTest {
    ...
    public void itStoresItsDamagePerTickAndCanReturnTheDamageSource() {
        assertEquals(17.5, subject.getDamageSource().getDamages().get(0).getDamageValue(), 0.0);
        assertEquals(TRUE, subject.getDamageSource().getDamages().get(0).getType());
    ...
    public void itReturnsTheTotalExpectedDamage() {
        ...
        assertEquals(122.5, totalDamage, 0.0);
    }
    ...
    private void setupDefaultSubject() {
        ...
        bleed.setDuration(fakeDuration);
        bleed.setNumberOfDamageTicks(fakeNumberOfTicks);
        bleed.setDamagePerTick(17.5);
        bleed.setDamageType(SLASH);
        bleed.setupTimers();
        when(statusFactory.getStatusProc(any(), eq(SLASH))).thenReturn(bleed);
        subject = (Bleed) statusFactory.getStatusProc(new DamageSource(PROJECTILE, Collections.singletonList(new Damage(SLASH, 50.0))), SLASH);
    }
}