...
public class PoisonTest {
    ...
    public void itStoresItsDamagePerTickAndCanReturnTheDamageSource() {
        ...
        assertEquals(TOXIN, subject.getDamageSource().getDamages().get(0).getType());
    ...
    private void setupDefaultSubject() {
        ...
        poison.setDuration(fakeDuration);
        poison.setNumberOfDamageTicks(fakeNumberOfTicks);
        poison.setDamagePerTick(fakeDamagePerTick);
        poison.setDamageType(TOXIN);
        poison.setupTimers();
        when(statusFactory.getStatusProc(any(), eq(TOXIN))).thenReturn(poison);
        subject = (Poison) statusFactory.getStatusProc(new DamageSource(PROJECTILE, Collections.singletonList(new Damage(TOXIN, 50.0))), TOXIN);
    }
}