...
public class HelperTest {
    ...
    public void itCanApplyDamageToHealth() {
        ...
        fakeTarget.setHealth(Collections.singletonList(fakeHealth));
        when(damageCalculatorMock.calculateDamage(any(Health.class), isNull(), isNull(), anyDouble(), any(Damage.class), anyDouble(), anyInt(), anyDouble())).thenReturn(50.0);

        Target modifiedFakeTarget = subject.applyDamageSourceDamageToTarget(fakeDamageSource, fakeHitProperties, fakeTarget);

        assertExpectedHealthExists(modifiedFakeTarget.getHealth(), new Health(HealthClass.MACHINERY, 150.0), 0.0);
        verify(damageCalculatorMock).calculateDamage(eq(fakeHealth), isNull(), isNull(), eq(fakeHeadshotMultiplier), eq(fakeDamage), eq(fakeWeaponCriticalDamageModifier), eq(fakeCritLevel), eq(fakeBodyPartModifier));
    }

    @Test
    public void itCanApplyDamageToShieldsAndHealth() {
        HealthMatcher deadShieldsMatcher = new HealthMatcher(new Health(HealthClass.SHIELD, 0.0));
        HealthMatcher startingShieldMatcher = new HealthMatcher(new Health(HealthClass.SHIELD, 200.0));
    ...
    public class HealthMatcher implements ArgumentMatcher<Health> {
